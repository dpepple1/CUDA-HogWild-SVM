...
#include "../include/SVM_multi_kernel.cuh"
#include <stdio.h>
#include <unistd.h>
...
__global__ void SGDKernel(uint threadCount, curandState_t *states, CSR_Data *d_data,
    ...
    int *labelStart = d_data->labels + (i * pairsPerThread) + subsetOffset; // Pointer to first label for thread
    int *sparsityStart = d_data->sparsity + (i * pairsPerThread) + subsetOffset;
    int subsetStart = i * pairsPerThread + subsetOffset; // Index to look in row start for row    
