...
#include "../include/SVM_sparse_hetero.cuh"
#include <stdio.h>
#include <unistd.h>
...
__host__ timing_t HOGSVM::fit(CSR_Data data, uint features, uint numPairs, int blocks, int threadsPerBlock, int tau)
{
    ...
    // Create a token in page mapped memory
    int *token = NULL;
    int *d_token = NULL;
    cudaHostAlloc(&token, sizeof(int), cudaHostAllocMapped);
    cudaHostGetDevicePointer(&d_token, token, 0);

    // Create a variable to track the synchronization status
    int *wait = NULL;
    int *d_wait = NULL;
    cudaHostAlloc(&token, sizeof(int), cudaHostAllocMapped);
    cudaHostGetDevicePointer(&d_wait, wait, 0);

    // Create a pointer to extract epoch
    int *epochBuf = NULL;
    int *d_epochBuf = NULL;
    cudaHostAlloc(&epochBuf, sizeof(int), cudaHostAllocMapped);
    cudaHostGetDevicePointer(&d_epochBuf, epochBuf, 0);s
    ...
    SGDKernel<<<blocks, threadsPerBlock, weightSize>>>(blocks * threadsPerBlock, states, d_data, 
                        ...
                        stepDecay, d_token, beta, lambda, blocks, tau, d_token, d_wait, d_epochBuf);
    cudaEventRecord(finished);

    float *thisActive = new float[features];
    float *thisSnapshot = new float[features];
    float *nextActive = new float[features];
    float *nextSnapshot = new float[features];
    cudaStream_t copyStream;
    cudaStreamCreate(&copyStream);

    // Code inside this loop will run on CPU for duration of the kernel
    while(cudaEventQuery(finished) != 0)
    {
        if ((*wait) == 0) //Time to perform synchronization step
        {
            // Copy data over to work on
            int next = (*token + 1) >= blocks ? (*token + 1) : 0;
            cudaMemcpyAsync(thisActive, activeWeights + (*token) * features, features * sizeof(float), cudaMemcpyDeviceToHost, copyStream);
            cudaMemcpyAsync(thisSnapshot, snapshotWeights + (*token) * features, features * sizeof(float), cudaMemcpyDeviceToHost, copyStream);
            cudaMemcpyAsync(nextActive, activeWeights + next * features, features * sizeof(float), cudaMemcpyDeviceToHost, copyStream);
            cudaMemcpyAsync(nextSnapshot, snapshotWeights + next * features, features * sizeof(float), cudaMemcpyDeviceToHost, copyStream);
            cudaStreamSynchronize(copyStream);

            for(int dim = 0; dim < features; dim++)
            {
                float weightDifference = thisActive[dim] - thisSnapshot[dim];
                // Refer to HogWild++ synchronization update formula
                thisSnapshot[dim] = lambda * nextActive[dim] + (1 - lambda) * thisSnapshot[dim] + beta * pow(stepDecay, *(epochBuf)*iter) * weightDifference;
                // Update next active weights
                nextActive[dim] +=  beta * pow(stepDecay, epoch *iter) * weightDifference; 
                thisActive[dim] = thisSnapshot[dim];
            }
            *wait = 1;
        }
        else //Synchronization has already been completed: waiting to pass to next
        {
...
__global__ void SGDKernel(uint threadCount, curandState_t *states, CSR_Data *d_data,
                            ...
                            float lambda, int blocks, int tau, int *d_token, int *d_wait, int *d_epochBuf) {   
    ...
    for(uint epoch = 0; epoch < epochs; epoch++)
    {
        ...
        for(uint iter = 0; iter < pairsPerThread; iter++)
        {
            ...
            if((*d_token) == blockIdx.x and threadIdx.x = 0)
            {
                if(*d_wait == 1)
                {
                    next_iter = (iter + tau) % pairsPerThread;
                    *d_wait = 2;
                }
                if(*d_wait == 2 and iter == next_iter)
                {
                    *d_wait = 0;
                    *d_epochBuf = epoch;
                }
...
__device__ __host__ void gpuAssert(cudaError_t code, const char *file, int line, bool abort)
{
   if (code != cudaSuccess) 
   {
