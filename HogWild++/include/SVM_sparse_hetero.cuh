...
CUDA_GLO void SGDKernel(uint threadCount, curandState_t *states, CSR_Data *d_data,
                            ...
                            float stepDecay, int *token, float beta, float lambda, int blocks, int tau, int *d_token, int *d_wait, int *d_epochBuf);
...
CUDA_HOS CUDA_DEV void gpuAssert(cudaError_t code, const char *file, int line, bool abort=true);
