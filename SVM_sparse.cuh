...
class HOGSVM
{
    public:
        ...
        // Public methods
        CUDA_HOS long fit(SparseDataset data, uint features, uint numPairs, int blocks,
                            int threadsPerBlock);
    ...
    private:
        ...
        CUDA_HOS thrust::device_vector<SparseEntry> *setupGPUPatterns(
                            thrust::host_vector<SparseEntry> *patterns, uint numPairs);
        CUDA_HOS void freeTrainingData(thrust::device_vector<SparseEntry> *d_patterns, int *d_labels);
};
...
CUDA_GLO void testKernel(thrust::device_vector<SparseEntry>* patterns, int* labels);
